...
function startAntlrProcess() {
	if (isShuttingDown) return;
	if (restartAttempts >= maxRestartAttempts) {
		console.error('Maximum restart attempts reached. Not restarting ANTLR process.');
		return;
	}
	const cliPath = path.resolve(__dirname, '../../configmate');
	antlrProcess = spawn('./bin/configm', ['serve'],
		{
			cwd: cliPath,
			detached: true,
			shell: true
	...
	if (!antlrProcess) console.error("ANTLR CLI failed to start");
	// else console.log(serverPath);
	antlrProcess?.stdout?.on('data', (data) => {
		console.table(data);
		// Process data from the CLI
	});
...
// Handle shutdown requests from the client
